...
public class DAO {
    ...
    public static Connection getConnection() {
        try {
            ...
            if(conn!=null){
                System.out.println("MySQL DB 접속성공");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void initGameSetting(){
        String query="update game_setting set level='중', question_num=10";
        try {
            stmt = conn.createStatement();
            stmt.executeUpdate(query);
        } catch (SQLException e) {
    ...
    public void saveGameData(String score, String level, String timeSpent, int quesNum, String date){
        String query = "update game set score=?, level=?, play_time=?, question_num=?, start_time=? where user_id=?";
        try{
            pstmt=conn.prepareStatement(query);
            ...
            pstmt.setString(6, GameManager.userID);
    ...
    public int countUserGameCode(String userID){
        int countGameCode=0;
        String query="select count(game_code) from game where user_id=?";
        try{
            pstmt=conn.prepareStatement(query);
